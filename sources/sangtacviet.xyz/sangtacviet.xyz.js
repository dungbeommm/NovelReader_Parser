/* NovelReader source: sangtacviet.xyz | Engine v4 (VBook + NovelReader APK cover/story). Auto-generated. */
(function (global) {
  "use strict";

  const config = {"logo":"https://sangtacviet.xyz/favicon.png","domain":"sangtacviet.xyz","keys":["sangtacviet","sangtacviet.xyz"],"use_html_parser":true,"use_webview":true,"min_android_version":129,"need_gecko":true,"download_warning":true,"title":{"selector":"#booknameholder"},"chapter":{"selector":"#bookchapnameholder"},"content":{"selector":".contentbox[cid]"},"content_replaces":["Đọc trên web để có chất lượng dịch cao"],"next_button":{"selector":"#navnexttop","value":"href","action":1},"pre_button":{"selector":"#navprevtop","value":"href","action":1},"data_loading_checker":[],"data_ready_checker":["Nhấp vào để tải chương","Đang tải nội dung chương"],"content_load_js":"var f=document.createElement('iframe');document.body.appendChild(f);XMLHttpRequest.prototype.send=f.contentWindow.XMLHttpRequest.prototype.send;document.body.removeChild(f);if(typeof chapterfetcher!=='undefined'){try{chapterfetcher.send('')}catch(e){console.log('STV send error:',e)}}","chapter_ignore_domain":false,"detail_first_chapter_js":"click:#readnowbtn","apply_word_segmentation":true,"detail":{"name":{"selector":"h1, .book-title, #bookinfo h1"},"author":{"selector":"a[href*='tacgia'], .author, #bookinfo a"},"description":{"selector":"#bookintro, .book-intro, .description"}},"cover":{"selector":"#bookimg img, .book img, .cover img","value":"src"}};
  const VERSION = 4;
  const ENGINE = 4;
