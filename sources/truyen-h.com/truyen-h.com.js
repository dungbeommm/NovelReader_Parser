/* NovelReader source: truyen-h.com | Engine v4 (VBook + NovelReader APK cover/story). Auto-generated. */
(function (global) {
  "use strict";

  const config = {"logo":"https://www.google.com/s2/favicons?sz=128&domain=truyen-h.com","domain":"truyen-h.com","keys":["truyen-h.com","www.truyen-h.com"],"use_html_parser":true,"use_webview":true,"title":{"selector":"h1.story-title"},"chapter":{"selector":"#chapterTitle"},"content":{"selector":"#chapterContent"},"next_button":{"selector":"a.th-next","value":"href"},"pre_button":{"selector":"a.th-prev","value":"href"},"content_load_js":"(function(){try{if(document.getElementById('th-ready'))return;var n=parseInt((new URL(location.href)).searchParams.get('chapter')||'0',10);function put(cls,num){if(num<1||document.querySelector('a.'+cls))return;var uu=new URL(location.href);uu.searchParams.set('chapter',num);var a=document.createElement('a');a.className=cls;a.href=uu.toString();a.style.display='none';document.body.appendChild(a);}if(n>0){put('th-next',n+1);put('th-prev',n-1);}var r=document.createElement('div');r.id='th-ready';r.style.display='none';document.body.appendChild(r);}catch(e){try{var r=document.createElement('div');r.id='th-ready';r.style.display='none';document.body.appendChild(r);}catch(_){}}})()","detail_first_chapter_js":"(function(){try{var id=(new URL(location.href)).searchParams.get('id');return id?location.origin+'/reader?story='+id+'&chapter=1':'';}catch(e){return '';}})()","data_loading_checker":["th-ready"],"chapter_ignore_domain":true,"enable_download":true};
  const VERSION = 4;
  const ENGINE = 4;
